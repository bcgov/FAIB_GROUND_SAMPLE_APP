...
server <- function(input, output, session) {
  ...
  sp_from_draw <- function(){
    if(!is.null(input$map_draw_all_features) & !is.null(drawnPolys()) & !is.null(sp_samplePoints_r()) ){
      p <- sp_samplePoints_r()
      s <- st_make_valid(st_as_sf(drawnPolys()))
      points <- p[st_intersects(p, s) %>% lengths > 0,]                  
  ...
  sp_samplePoints_r <-reactive({
    ...
    if(length(input$tsa) > 0 & length(input$bec) > 0 & (length(input$cbSType) > 0 || length(input$sts) > 0)){
      if(input$prj == 'All'){
  ...
  masterTable <- reactive({
    ...
    else if((length(input$cbSType) > 0 || length(input$sts) > 0) & length(input$prj) > 0 & length(input$tsa) > 0 &  length(input$bec) > 0 & !is.null(sp_samplePoints_r())){
...
# 
  # Help for Plots
  spHelpModalGen = modalDialog(
    title = HTML("<h2><b>Plot types with generalized coordinates</b></h2>"),
    easyClose = TRUE,
    fade = FALSE,
    HTML("Generalized coordinates are provided for the three plot types that make up the Provincial Monitoring Program so that unbiased change on the landscape can be monitored over time. 
          
          <h4><b>Change Monitoring Inventory (CMI) plots:</b></h4> Provide statistically-sound, point-in-time and change estimates of vegetation attributes in mature stands (>50 years old) over existing sampled areas of the province with fixed-radius plots located on the 20 km by 20 km NFI grid. 
          
          <h4><b>Young Stand Monitoring (YSM) plots::</b></h4> Monitor the growth and health of young (15-50 years old) stands using fixed-radius plots located on an intensification of the 20 km by 20 km NFI grid.
  ...
  spHelpModalAct = modalDialog(
    title = HTML("<h2><b>Plot types with actual coordinatess</b></h2>"),
    easyClose = TRUE,
    fade = FALSE,
    HTML("Actual coordinates are provided for the active and inactive PSPs and for the VRI plots.
  ...
  projDesHelpModal = modalDialog(
    title = HTML("<h2><b>Project Design</b></h2>"),
    easyClose = TRUE,
    fade = FALSE,
    HTML("<h4><b>YSM Main</b></h4> YSM, CMI, or NFI plots that are on the main YSM grid (usually 5x10km) that are in the young stand population (15-50yrs old) based on the phase 1 inventory age at time of first measurement.

    <h4><b>Mature Main</b></h4> CMI, NFI, or SUP plots that are on the main CMI grid (usually 20x20km) that are in the mature population (>50yrs old) based on the phase 1 inventory age at time of first measurement.<br><br>")
  )
  ...
  observeEvent(input$samplePlotsHelpGen, {
    showModal(spHelpModalGen)
  })
  
  observeEvent(input$samplePlotsHelpAct, {
    showModal(spHelpModalAct)
  })
  
  observeEvent(input$projDesHelp, {
    showModal(projDesHelpModal)
  })
